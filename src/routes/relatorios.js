...
router.get("/animais", async (req, res) => {
  try {
    ...
    const disponiveis = await prisma.animal.count({
      where: { status: "disponivel" }
    });

    const adotados = await prisma.animal.count({
...
router.get("/usuarios", async (req, res) => {
  try {
    ...
    const ongs = await prisma.usuario.count({
      where: { tipo: "ong" }
    });

    const normais = await prisma.usuario.count({
      where: { tipo: "normal" }
    });
